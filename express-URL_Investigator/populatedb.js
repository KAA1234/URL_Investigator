...
console.log(
  'This script populates the Domains and VirusTotal collection to the mongoDB database. Specified database as argument - e.g.: node populatedb "mongodb+srv://cooluser:coolpassword@cluster0.lz91hw2.mongodb.net/local_library?retryWrites=true&w=majority"'
);
...
const VirusTotal = require("./models/virustotal");
...
async function createDomains() {
  ...
  await Promise.all([
    domainsCreate("google.com"),
  ]);
}

async function createVirusTotal() {
  console.log("Adding VirusTotal");
  await Promise.all([
    virusTotalCreate("score=test")
  ]);
}
/* 
